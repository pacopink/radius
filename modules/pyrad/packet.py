...
class Packet(dict):
    ...
    def CalculateAuth(self):
        assert(self.secret)
        header = self.raw_packet[0:4]
        attr = self.raw_packet[20:]
        #part1 = struct.pack('!BBH', self.code, self.id, len(attr))
        #Request Authenticator = MD5(Code+Identifier+Length + byte 0 * 16 + Attributes + Secret).
        authenticator = md5_constructor(header + struct.pack("B", 0)*16  + attr + self.secret).digest()
        return authenticator

    def CalculateAuthRsp(self, auth):
        assert(self.secret)
        header = self.raw_packet[0:4]
        attr = self.raw_packet[20:]
        #part1 = struct.pack('!BBH', self.code, self.id, len(attr))
        #Request Authenticator = MD5(Code+Identifier+Length + byte 0 * 16 + Attributes + Secret).
        authenticator = md5_constructor(header + auth  + attr + self.secret).digest()
        return authenticator
...
class AuthPacket(Packet):
    ...
    def RequestPacket(self):
        ...
        if self.authenticator is None:
            ...
            header = struct.pack('!BBH', self.code, self.id, len(attr))
            self.authenticator = md5_constructor(header + struct.pack("B", 0)*16  + attr + self.secret).digest()
