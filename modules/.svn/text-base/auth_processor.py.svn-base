...
def init_sig():
    def signal_handler(sig, frame):
        ...
        if sig == signal.SIGUSR1:
            ...
            if auth_mgr:
                auth_mgr.reload_redis_buffers()
    signal.signal(signal.SIGINT, signal_handler)
    signal.signal(signal.SIGTERM, signal_handler)
    signal.signal(signal.SIGUSR1, signal_handler)


class AuthDipcMgr(EndPointMgr):
    def __processing__(self, msg):
        ...
        if msg2.msg_type == MT_AUTT_2_AUTP:
            ...
            try:
                ...
                pkt = packet.AuthPacket(dict=dict_obj, packet=msg2.body)
                if pkt.has_key('State'):
                    ...
                    state = pkt['State'][0]
                    auth_mgr.resume_task(state, msg2.msg_type, msg2.body, sender=msg2.sender, subkey=msg2.header)
                else:
                    auth_mgr.new_task(key, msg2.host, msg2.sender, pkt, kpi_ticker)
            except Exception,e:
                kpi_ticker.increase_kpi('ACCESS_INVALID')
                #print "Process radius request exception %s:%s"%(type(e), e)
                print ("%s: %s"%(e, traceback.format_exc()))
                return 
        elif msg2.msg_type == MT_PASS_2_AUTP or msg2.msg_type == MT_HLR_2_AUTP:
            #when adapter returned with a result of password retrieval
            key = msg2.header.encode('hex')
            auth_mgr.resume_task(key, msg2.msg_type, msg2.body) 
...
def init_logger(args):
    log = logger()
    log_path = os.path.join(args.logpath, "auth_processor_%s.log"%args.ln)
    ret = log.init(log_path, "auth_processor", args.ln)
    if ret != SUCCESS:
...
def msg_loop():
    ...
    while not terminate and not ep_mgr.dead and not cdr_writer.dead:
        ...
        #check if idle sleep
        chocker.idle_switch(ep_mgr.idle and auth_mgr.idle)
        now = time.time()
        if now - last_time >= 10:
            log.write_log(INFO, SUCCESS, "Authentication Task: %d"%len(auth_mgr.task_dict.keys()))
...
def main():
    global args, log, rp, ep_mgr, cdr_writer, auth_mgr, kpi_ticker
    try:
        args = init_args()
        init_sig()
        ...
        ep_mgr = AuthDipcMgr(my_dipc_type, args.ln, concern_type_list, rp, log) 

        if not ep_mgr.start():
            log.write_log(ERROR, SUCCESS, "Failed to start EndPointMgr")
            sys.exit(1)

        cdr_writer = radius_cdr_writer.AuthCdrWriter(path=args.cdr, filename_pattern="AUTH_%s_$TIMESTAMP.txt"%args.ln)
        auth_mgr = AuthenticateTaskMgr(redis_pool=rp, ep_mgr=ep_mgr, cdr_writer=cdr_writer, log=log)
